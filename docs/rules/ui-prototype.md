...
Frontend UI changes require a **module-organized prototype set** (`index.html` + `stories.md` + `flows.md` + page HTML files) under `docs/design/prototypes/<module>/` before FE/BE/SYNC contracts are locked.
...
2. Ensure the module ships the mandatory set: `index.html`, `stories.md`, `flows.md`, and ≥1 page HTML file.
3. Update the files to reflect the REQ's target behavior.
4. Confirm every page HTML file carries the 4-field header (see §5).
5. Confirm `stories.md` and `flows.md` satisfy §6 and §7.
6. Link the module prototype set into FE/BE/SYNC contracts before contract lock (by directory path + fingerprint of the current contents).
...
Every page HTML file carries a 4-field header in a dark gradient `.proto-meta` bar. **No other fields.**

| # | Field | Format | Example |
|:---|:---|:---|:---|
| 1 | 设计代数 | `v<int>` | `v1`, `v2` — bumps only on major redesign |
...
- tracking prototype file versions (semver, addendum, hotfix-addendum) — only the 设计代数 label is allowed, and it bumps only on major redesign
