...
F5 只用最小而有张力的样本证明语言可辐射。模块 `prototypes/` 证明当前业务全集；本目录证明 Kernel/Grammar 能否在真实业务和压力下成立。
...
位于 Moodboard 与整页稿之间。静态 HTML，不暗示设备尺寸。每个候选方向一份：
`style-tiles/direction-a.html`。从 `STYLE-TILE-template.html` 复制。

每张 Tile 至少包含：版式与字体关系、色彩角色、表面层级、图像/图标/材质、主次行动与状态、一段真实内容、一个成功态、一个失败或空态、对应的 Thesis / Laws / Anti-principles / 风险。

选定后保留落选文件，供方向 ADR 追溯。人选择的是设计思想，不是“哪张图最好看”。

F2 Tile 可以使用候选 hex。F6 之后的 Anchor / Stress / Golden Flow 与模块原型必须引用 `packages/design-tokens/tokens.css`，不得再发明未登记色值。

## Anchor / Stress / Golden Flow

使用与模块原型相同的四字段 HTML 头（见 `docs/rules/ui-prototype.md`），但存放于此，不替代 `prototypes/<module>/`。
