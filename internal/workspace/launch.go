...
func (b *ExecutionRegistry) Launch(ctx context.Context, e Execution, binary, role, prompt string, resume bool, stdout, stderr io.Writer) error {
	...
	contextData, _ := json.Marshal(map[string]any{"assignment": e, "frozen_input_directory": ".claude/inputs", "begin_command": WorkerInvocation(e, "begin"), "commit_command": WorkerInvocation(e, "commit"), "report_command": WorkerInvocation(e, "report"), "s9_delivery_command": WorkerInvocation(e, "deliver")})
	prompt += "\nRegistered Harness context: " + string(contextData) + "\nRead approved input documents under .claude/inputs. Keep the Main checkout unchanged. Write lifecycle submissions under .claude/submissions: plan.json for begin, completion.json for report; use the repository agent-protocol schemas. commit.json requires expected_head, message and explicit paths. S9 repair-result.json is a candidate only; report then deliver. Use only the exact registered adapters for Bash; executable check commands are shown by workspace prepare. A process exit is not completion or release approval."
