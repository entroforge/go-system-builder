...
func printActionCatalog(w io.Writer) {
	...
	fmt.Fprintln(w, "Use the first applicable action; command output names the next action. Stage commands own their Runtime writes; do not use `runtime transition` directly.")
	fmt.Fprintln(w)
	...
	fmt.Fprintln(w, "  `loop-harness runtime repair handoff create --file <handoff-request.json>` → `runtime repair handoff commit --file <handoff.json> --actor <agent>` — the only repair exit; return to a fresh S7 round")
	fmt.Fprintln(w)
