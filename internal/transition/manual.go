...
func writeControlPlaneRecovery(b *strings.Builder) {
	...
	fmt.Fprintf(b, "5. `SubagentStop` returns a short integration action. Main runs `runtime task-integrate --assignment-id <id> --root <main-root>` to inspect, merge into the explicitly registered main branch, check, persist completion_ack, and clean up. No develop fallback or automatic Main checkout. On `TeammateIdle`, inspect current progress and re-wake the same teammate when appropriate.\n")
