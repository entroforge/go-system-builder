...
> 层结构（以第一层 §六 的唯一权威定义为准）：第一层 = 工程哲学与设计蓝图；**本文 = 第二层 = 生命周期整体规划（实战目标层）**；第三层 = 各 Stage 详细设计（L3-S0…L3-S11，含工具承载）；第四层 = 跨 Stage 工具机制与治理设计。
...
全局交付与来源规则：每一阶段的主要 Markdown、代码和测试产出及时提交；未提交内容不能支撑下一阶段资格。上游输入契约逐项选择 Git tree、磁盘 evidence 或 Runtime 快照；具体读取与事务规则由 [L4 状态机核心 §5.4](../l4-mechanisms/L4-state-transition-core.md#54-上游输入契约与文件视图)定义。

REQ 在 S1 显式绑定开发主分支与最终发布上游；worktree 从开发分支已提交基线创建，子分支提交后以合并提交回到项目根目录，验证接收后清理。未回收、分支偏离和资源积压只提醒 Agent；必要阶段产出尚未进入声明的 Git tree 时阶段条件仍不满足。权威机制见 [L4 Worktree](../l4-mechanisms/L4-worktree-governance.md)。
...
确立已提交阶段交付、上游来源契约与 REQ 分支/临时 worktree 全局规则。


## 共享模型与合同依赖（2026-09-19）

S3 的共同数据模型与 SYNC 协议按业务切片先收敛，再派生 FE/BE 责任；S4 以真实产物依赖拆分任务，S5 冻结共同设计闭包，S6/S7 验证实际同源消费。 机制权威见 [L4 共享模型与合同治理](../l4-mechanisms/L4-shared-model-contract-governance.md)。


## S4—S6 派发计划契约（2026-09-19）

S4 交付、S5 审签、S6 持续消费同一整体派发计划。波次不是全局屏障；容量不足只排队，不改变依赖或删减任务。静态计划与实时执行投影分离，机制权威见 [L4 整体派发计划](../l4-mechanisms/L4-agent-dispatch-governance.md#dispatch-plan)。本修订为目标设计，当前实现边界见各 L3 当前事实节。
