...
> **Revision 口径**：Runtime `revision` 是 Writer 内部生成的提交序号，不是绑定授权、人类决策或 Agent 的正常输入。主会话提交候选 REQ、授权人、开发主分支、发布上游和业务意图；锁、原子写、journal、pending 及 revision 分配由 Runtime Writer 完成。详见 [L4 Runtime revision 使用与命令协调](../l4-mechanisms/L4-revision-usage.md)。
...
普通 Hook 不改变绑定，分支偏离或成果尚未回收只提示主会话。历史绑定缺少分支时显式补全，不猜默认。amend 保留已有 workspace 绑定；变更目标必须显式声明并留审计。详细身份与生命周期见 [L4 Worktree](../l4-mechanisms/L4-worktree-governance.md)，文件读取见 [L4 状态机核心 §5.4](../l4-mechanisms/L4-state-transition-core.md#54-上游输入契约与文件视图)。
...
绑定原子登记显式开发/发布分支，并从指定 Git tree 发现和绑定 REQ。
