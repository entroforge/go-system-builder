...
For new Builder assignments explicitly set `integration_check_mode: post_merge` in the immutable manifest. Inspect then performs static preconditions; required_checks run on the actual integration target after merge and still block verified on failure. Do not redirect those commands to the source worktree. Omitted mode retains legacy pre-and-post execution for existing manifests; unknown values fail closed. Never edit an active manifest to change modes. Any intentional pre-merge baseline diagnosis is a separate, purpose-labelled check, not a second implicit run of the delivery suite.
