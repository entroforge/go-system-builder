...
func TestScanDetectsUnknownStatusInTemplate(t *testing.T) {
	...
	policy, err := os.ReadFile("../../docs/control/hook-policy.json")
	if err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, "docs/control/hook-policy.json"), policy, 0o644); err != nil {
		t.Fatal(err)
	}
	...
	for _, rel := range []string{"docs/dev/contracts/CONTRACTS-template.md", "docs/dev/contracts/BE-contract-template.md", "docs/dev/contracts/FE-contract-template.md", "docs/dev/contracts/SYNC-contract-template.md", "docs/architecture/ARCHITECTURE-template.md", "docs/dev/tasks/index-template.md"} {
...
// catches when docs/control/hook-policy.json names a Claude Code event the
...
func TestScanFlagsUnregisteredProtectedEvent(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, "docs/control/hook-policy.json"), []byte(policy), 0o644); err != nil {
		t.Fatal(err)
	}
	schema, err := os.ReadFile("../../internal/schema/assets/hook-policy.schema.json")
	if err != nil {
...
func TestMissingRequiredTemplatesFail(t *testing.T) {
	report := scan(t.TempDir())
	count := 0
	for _, f := range report.Findings {
		if f.Category == "required-template-missing" {
			count++
		}
	}
	if report.OK() || count < 7 {
		t.Fatalf("missing templates silently skipped: %+v", report.Findings)
	}
}
